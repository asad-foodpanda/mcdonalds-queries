...
with data as (select count(distinct case when o.decline_reason.title in ("Restaurant too busy","Restaurant not reachable","Restaurant does not deliver to area","Restaurant closed") then o.code end) as unfulfilled_orders,
...
select unfulfilled_orders / gross_orders as unfulfilled from data