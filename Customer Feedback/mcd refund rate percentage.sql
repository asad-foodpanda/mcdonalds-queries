...
where v.global_entity_id = 'FP_PK'
  ...
  and o.created_date_local between start_date and end_date
  and o.decline_reason.title in ("Customer received food in inedible condition","Customer received food totally spilled","Customer received order too late (>1h)","Customer received wrong order")
)
select 
safe_divide(
  count(distinct case when o.code in (select code from inaccurate_orders) then o.code end),
  count(distinct case when o.is_valid_order then o.code end)
  )
from `fulfillment-dwh-production.pandata_curated.pd_orders` o
join `fulfillment-dwh-production.pandata_curated.pd_vendors` v
  on o.vendor_code = v.vendor_code and o.global_entity_id = v.global_entity_id 
